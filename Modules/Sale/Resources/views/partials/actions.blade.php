<div class="btn-group dropleft">
    ...
    <div class="dropdown-menu">
        ...
        <a target="_blank" href="{{ route('sales.pos.invoice', $data->id) }}" class="dropdown-item">
            <i class="bi bi-file-earmark-pdf mr-2 text-success" style="line-height: 1;"></i> {{__('messages.print_struk')}}
        </a>

        @can('access_sale_payments')
            <a href="{{ route('sale-payments.index', $data->id) }}" class="dropdown-item">
                <i class="bi bi-cash-coin mr-2 text-warning" style="line-height: 1;"></i> {{__('messages.payment')}}
            </a>
        @endcan
        @can('access_sale_payments')
            @if($data->due_amount > 0)
            <a href="{{ route('sale-payments.create', $data->id) }}" class="dropdown-item">
                <i class="bi bi-plus-circle-dotted mr-2 text-success" style="line-height: 1;"></i> {{__('messages.add')}}  {{__('messages.payment')}}
            </a>
            @endif
        @endcan
        @can('edit_sales')
            <a href="{{ route('sales.edit', $data->id) }}" class="dropdown-item">
                <i class="bi bi-pencil mr-2 text-primary" style="line-height: 1;"></i> {{__('messages.edit')}}
            </a>
        @endcan
        {{-- @can('show_sales') --}}
        ...
        @can('delete_sales')
            <button id="delete" class="dropdown-item" onclick="
                ...
                <i class="bi bi-trash mr-2 text-danger" style="line-height: 1;"></i> {{__('messages.delete')}}
